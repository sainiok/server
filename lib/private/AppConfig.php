...
class AppConfig implements IAppConfig {
	...
	private array $valueTypes = [];  // cache for normal and lazy loaded config keys
	 ...
	 * @param bool $filtered TRUE to filter sensitive config values
	 *
	...
	public function convertTypeToInt(string $type): int {
		try {
			...
			throw new AppConfigIncorrectTypeException('Unknown type ' . $type);
		}
	}
	...
	public function convertTypeToString(int $type): string {
		...
		try {
			...
			throw new AppConfigIncorrectTypeException('Unknown numeric type ' . $type);
		}
	}
